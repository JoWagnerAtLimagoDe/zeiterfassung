...
public class ZeitmodellController {
	...
	public String zeitmodell(@RequestParam(value = "showMessage", required = false, defaultValue = "false") boolean showMessage, Model model) {
		try {
			if (! showMessage)
				messages.clear();
	...
	public String anzeigenZeitmodell(@PathVariable String id, Model model) {
		try {
			...
			Zeitmodell zeitmodell = optionalModell.orElseThrow(()->new ZeitmodellServiceException("Modell nicht gefunden!"));
	...
	public String saveZeitmodell(@Valid @ModelAttribute Zeitmodell zeitmodell,BindingResult bindingResult, Model model) {
		try {
